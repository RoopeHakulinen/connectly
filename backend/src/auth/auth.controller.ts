...
export class AuthController {
  ...
  @Get('redirect')
  @Redirect('', 302)
  async redirectToOauth(@Query('redirect_url') appRedirectUrl: string) {
    const state = appRedirectUrl ?? this.appUrl;
    const clientId =
      '372875022616-vi137ar0f85rt6e1rgc8fk94t35h062p.apps.googleusercontent.com';
  ...
  async oauthLoginRedirect(
    ...
    if (!code) {
      throw new UnauthorizedException(
        'The code is missing on the Google authentication callback.',
      );
    }
    ...
    const name = (tokenData as any).name;
  ...
  private async exchangeCodeForToken(code: string): Promise<string> {
    ...
    const openIdResponse: any = await firstValueFrom(
      this.http.post<any>('https://oauth2.googleapis.com/token', {
        code,
